...
export default defineConfig(({mode}) => {
    ...
    // Vite rejects requests whose Host header is a name rather than an IP. A leading dot
    // matches the domain and its subdomains, so MagicDNS names reach the dev server.
    const allowedHosts = ['.ts.net', ...(env.MW_ALLOWED_HOSTS || '').split(',').filter(Boolean)];
    return {
        ...
        server: {host: '0.0.0.0', port: Number(env.PORT || 8601), cors: true, allowedHosts,
            watch: {ignored: ['**/deploy-build/**']},
            fs: {allow: [path.dirname(directory)]}},
        preview: {port: Number(env.PORT || 8601), allowedHosts},
        optimizeDeps: {
