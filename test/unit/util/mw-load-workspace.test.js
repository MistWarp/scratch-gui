import loadWorkspace from '../../../src/lib/mw-load-workspace';
...
test('collaboration keeps every block available to editing commands', () => {
    const {Xml, data, dom} = setup(100);
    loadWorkspace({Xml}, {}, data, {}, false);
    expect(Xml.clearWorkspaceAndLoadFromXmlDeferred).not.toHaveBeenCalled();
    expect(Xml.clearWorkspaceAndLoadFromDescs).toHaveBeenCalledWith(dom, data.blocks, {});
});
