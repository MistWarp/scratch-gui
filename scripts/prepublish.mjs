...
const downloadMicrobitHex = async () => {
    ...
    const sha256 = nodeCrypto.createHash('sha-256')
        .update(zipBuffer)
        .digest('hex');
    ...
    const relativeGeneratedFile = path.join(relativeGeneratedDir, 'microbit-hex-url.js');
    ...
    const publicPath = `microbit/${hexFileName}`;
    fs.writeFileSync(
        absoluteGeneratedFile,
        [
            '// This file is generated by scripts/prepublish.mjs',
            '// Do not edit this file directly',
            '// Path is relative to the site root (static/ is served at the root).',
            '// process.env.ROOT is baked in at build time (see vite.config.mjs).',
            `const hexUrl = \`\${process.env.ROOT}${publicPath}\`;`,
            '',
