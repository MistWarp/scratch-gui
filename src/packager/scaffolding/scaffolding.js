...
import {openGameShop} from './game-shop';
...
class Scaffolding extends EventTarget {
  ...
  loadProject (data) {
    this.accountHost = createAccountHost(this.vm.runtime, {...this.accountOptions, openShop: openGameShop});
    return this.vm.loadProject(data)
      .then(async () => {
        await this.accountHost.prepare();
