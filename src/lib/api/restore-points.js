...
const STORAGE_UNAVAILABLE = 'StorageUnavailableError';

const storageUnavailableError = detail => {
    const error = new Error(`Device backups are not available here. ${detail}`);
    error.name = STORAGE_UNAVAILABLE;
    return error;
};

const isStorageUnavailableError = error => Boolean(error) && error.name === STORAGE_UNAVAILABLE;
...
const openDB = () => {
    ...
    if (typeof indexedDB === 'undefined') {
        return Promise.reject(storageUnavailableError('IndexedDB is not available'));
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            reject(storageUnavailableError(error.message || String(error)));
            return;
        }

        request.onblocked = () => {
            reject(new Error('Could not open database: another tab is still upgrading it'));
        };
        ...
        request.onerror = () => {
            ...
            if (error && (error.name === 'SecurityError' || error.name === 'InvalidStateError')) {
                reject(storageUnavailableError(error.message));
                return;
            }
            reject(new Error(`Could not open database: ${error}`));
        };
    });
};

const abortsTransaction = (transaction, reject, action) => {
    transaction.onabort = event => {
        const error = (event && event.target && event.target.error) || transaction.error;
        reject(new Error(`${action}: ${error ? error.message || error : 'the transaction was aborted'}`));
    };
};
...
const createRestorePoint = (
    ...
    generateThumbnail(vm).then(thumbnailData => {
        ...
        abortsTransaction(transaction, error => {
...
const deleteRestorePoint = id => openDB().then(db => new Promise((resolve, reject) => {
    ...
    abortsTransaction(transaction, reject, 'Deleting restore point');
...
const deleteAllRestorePoints = () => openDB().then(db => new Promise((resolveTransaction, rejectTransaction) => {
    ...
    abortsTransaction(transaction, rejectTransaction, 'Deleting all restore points');
